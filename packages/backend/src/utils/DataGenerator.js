...
const htmlType = __importStar(require("@quatrain/core/lib/properties/types/PropertyHTMLType"));
...
const DataGenerator = (model, qty = 5, forcedValues = {}) => __awaiter(void 0, void 0, void 0, function* () {
    const promises = [];
    Backend_1.Backend.log(`Starting to create ${qty} ${model.constructor.name} records`);
    for (let i = 0; i < qty; i++) {
        const dao = yield model.dataObject.clone();
        Object.keys(dao.properties).forEach((key) => {
            Backend_1.Backend.log(`Getting property for '${key}'`);
