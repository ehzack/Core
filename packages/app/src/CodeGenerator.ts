...
`
   ...
   private static generatePackageJson(config: any, targetDir: string): void {
      ...
      const packageJson = {
         ...
         version: "1.0.0",
         description: "Generated by Core Studio",
         main: "src/index.ts",
         type: "module",
         scripts: {
...
${config.authMode === 'oauth' ? `      // Initialize OIDC Auth
      const oidc = AuthOIDC.factory('http://localhost:4001')
      server.getNativeInstance().use('/oidc', oidc.callback())
` : ''}
${config.authMode === 'basic' ? `      // Initialize Basic Auth
      const basicAuth = AuthBasic.factory(process.env.APP_AUTH_USER, process.env.APP_AUTH_PASS)
...
`
   ...
   private static generateFrontend(config: any, targetDir: string, models: string[]): void {
      const appName = config.name?.toLowerCase().replaceAll(/[^a-z0-9]/g, '-') || "quatrain-app"
