...
- **`abstract class AbstractMdmObject extends PersistedBaseObject`**: Core abstract model class that MUST be extended to define concrete real-world objects (e.g. `GarmentMdmObject`, `DiskMdmObject`, `HardwareDeviceMdmObject`, `VirtualKeychainMdmObject`). Cannot be instantiated directly.
